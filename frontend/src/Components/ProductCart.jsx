...
const Product = ({ items }) => {
  ...
  return (
    <>
      <div className="flex lg:justify-start flex-wrap gap-x-[2%] md:gap-x-[6%] lg:gap-x-[2%] xl:gap-x-[1.3%] gap-y-2 md:gap-y-8 lg:gap-y-4 ">
        {currentItems.map((item) => (
            // Cards
          <Link
            to={`/category/${item.category}/${item.title}/${item.description}/${item._id}`}
            className="flex flex-col justify-between w-[49%] md:w-[47%] lg:w-[32%] xl:w-[24%] rounded-lg border-2 border-[#d4d5d9] px-1.5 py-1.5 sm:px-2 sm:py-2 transition-transform duration-300 hover:scale-105"
            key={item._id}
          >
      ...
      {/* Pagination Controls */}
      <div className="flex gap-3 sm:gap-6 justify-center items-center mt-12">
        {/* Previous Button */}
        <button
